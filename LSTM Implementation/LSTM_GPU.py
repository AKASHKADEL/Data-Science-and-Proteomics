...
human_train_idx, human_valid_idx, human_test_idx, human_train_labels,human_valid_labels,     human_test_labels, human_GO_terms     = dp.load_test_sets('../capstone_data/human_annotations_temporal_holdout.mat')
...
yeast_sequences, yeast_protein_names = dp.load_FASTA('../capstone_data/yeast_sequences.fasta')
yeast_train_idx, yeast_valid_idx, yeast_test_idx, yeast_train_labels, yeast_valid_labels,    yeast_test_labels, yeast_GO_terms = dp.load_test_sets('../capstone_data/yeast_MF_temporal_holdout.mat')
...
def train_test(num_epochs,optimizer,train_iter,eval_batches,model,training_length):
    losses = []
    ...
    for epoch in range(1,num_epochs+1):
        stop_training = False
        for i, (train_data, train_labels,_) in enumerate(train_iter):
            model.train()
            model.zero_grad()
            ...
            # The below version only checks the first 1 of 38 dev batches per evaluation
            if (i+1) % 1 ==0:
                model.eval()
                hidden, c_t = model.init_hidden()
                ...
                val_loss = criterion(val_outputs.data.round(), valid_labels.float())
                valid_loss_history.append(val_loss)
                print('Epoch: [{}/{}], Step[{}/{}], Average Train Loss: {}, Validation Loss: {}'.                      format(epoch, num_epochs, i+1, total_batches, np.mean(losses),val_loss))
            
            if (i+1) % total_batches == 0:
                torch.save(model.state_dict(), '{}Model{}Epoch'.format(organism,epoch)) # Saves model after every epoch   
